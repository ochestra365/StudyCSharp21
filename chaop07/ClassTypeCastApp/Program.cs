    ...
    //발생하는 현상이다.
{
    ...
    class Program//상호작용하는 것을 표현하는 필드, 현실세계로 보자면 전체적인 객체들이 상호작용하는 공간에 해당한다.
    {
        static void Main(string[] args)//계속 지속되고, 종료시 결과값이 도출될 수도 없고 의미도 가지지 않으며, 필드에 선언 및 할당하는 문자열들은 무제한이다.[]-->배열
        {
            ...
            // 클래스 간의 형변환은 로직의 작업 효율성을 높이기 위해서이다. 왜냐하면 상위클래스일 수록 추상화가 높고 하위클래스일수록 일반화 되어 있어서 개발자가 달성하고자 하는 기능을 작업 효율을 높일 수 있기 때문이다.
            ...
            //메인 메서드에서 형변환 해주는 이유는 참조클래스의 필드를 건들지 않고, 달성하고자 하는 기능을 실현하기 위해서 하는 것이고, 또한 코드제어를 일반화 시켜서 더 효율화하기 위함이다.
            //물리적 소스제어를 보고 그것을 분석해보는 것이 본 과정을 깊게 이해하는 것에 도움을 줄 거 같다.
            //클래스를 직접 건드리는 것은 완성된 시계 구조를 건드리는 것이고, 각 클래스들은 서로 엮여서 복합적으로 참조하고 있기 때문에 하나를 바꾸면 시스템 전반적인 에러가 발생할 가능성이 매우 높아진다.
        }
    }
}
